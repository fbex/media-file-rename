...
def find(media_item: TvMediaItem) -> TmdbItem:
    ...
    verbose_print(
        "Found tmdb item: {}, {}, {}".format(name, media_type, tmdb_id))
    if name != media_item.name:
        print(
            "No media information found for title [{}]".format(
                media_item.name))
...
def get_episodes_for_season(
        tmdb_item: TmdbItem, season_number: str) -> List[TvEpisode]:
    tv_url = '{url}/tv/{tv_id}/season/{season_number}' \
        .format(url=_tmdb_url, tv_id=tmdb_item.id, season_number=season_number)
    params = {
        "api_key": _tmdb_api_key,
        "language": "en-US"
    }
    ...
    return [TvEpisode(number=episode['episode_number'], name=episode['name'])
            for episode in episodes]


def find_episode_by_number(number: int, tv_episode: List[TvEpisode]) -> \
        Optional[TvEpisode]:
...
def _handle_request_error(r: Response):
    if r.status_code != 200:
        verbose_print(
            "Error obtaining media information, because: {}".format(r.reason))
        verbose_print(
            "Response was [{}] with content: [{}]".format(r.status_code,
                                                          r.json()))
